...
Program::Program(core::shared_ptr<Renderer_Base> rendererBase, flecs::world world)
  ...
  , m_cameraNode(world, "camera node")
{
  ...
  resources::GLTF_Loader gltfLoader(m_rendererBase->getTransferCommandPool(), world);
...
void Program::initalizeDescriptorSet()
{
  core::array<Descriptor_Set_Layout_Binding, 7u> layoutBindings = m_programDescriptors.getDescriptorLayoutBindings();
  core::array<Descriptor_Pool_Size, 7u> poolSizes = m_programDescriptors.getDescriptorPoolSizes();
  m_descriptorPool = core::allocateShared<Descriptor_Pool>(m_rendererBase->getDevice(), poolSizes);
...
core::shared_ptr<Render_Pass> Program::initializeRenderPass()
{
...
void Program::recordCommandBuffer(core::shared_ptr<Command_Buffer> commandBuffer, core::uint32 imageIndex)
{
  ...
  renderComponent.perFrameUniformBufferDescriptor = &m_programDescriptors.m_perFrameUniformBufferDescriptor[m_currentFrame];
  renderComponent.textureSamplerDescriptor = &m_programDescriptors.m_textureSamplerDescriptor[m_currentFrame];
  renderComponent.submeshesDrawsBufferDescriptor = &m_programDescriptors.m_submeshesDrawsBufferDescriptor;
  renderComponent.vertexBufferDescriptor = &m_programDescriptors.m_vertexBufferDescriptor;
  renderComponent.meshletBufferDescriptor = &m_programDescriptors.m_meshletBufferDescriptor;
  renderComponent.primitiveIndicesBufferDescriptor = &m_programDescriptors.m_primitiveIndicesBufferDescriptor;
  renderComponent.vertexIndicesBufferDescriptor = &m_programDescriptors.m_vertexIndicesBufferDescriptor;
...
void Program::initalizeGraphicsPipeline()
{
...
void Program::initializeColorResources()
{
...
void Program::initializeDepthImage()
{
  ...
  auto commandBuffer = core::allocateShared<Command_Buffer>(m_rendererBase->getTransferCommandPool(), VK_COMMAND_BUFFER_LEVEL_PRIMARY);
...
void Program::initializeSwapchainFramebuffers()
{
...
void Program::initializeUniformBuffers()
{
  for (core::uint32 i = 0; i < cFramesCount; ++i)
  {
    ...
    m_programDescriptors.m_perFrameUniformBufferDescriptor[i].update(m_perFrameUniformBuffer[i]);
  }
}

void Program::initializeCommandBuffers()
{
  for (core::uint32 i = 0; i < cFramesCount; ++i)
  {
    m_graphicsCommandBuffers[i] = core::allocateShared<Command_Buffer>(m_rendererBase->getGraphicsCommandPool(), VK_COMMAND_BUFFER_LEVEL_PRIMARY);
  }
}

void Program::initializeSyncObjects()
{
  for (core::uint32 i = 0; i < cFramesCount; i++)
  {
